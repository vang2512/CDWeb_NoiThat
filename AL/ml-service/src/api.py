...
try:
    ...
    # Load Sentiment Model (PhoBERT)
try:
    sentiment_model = SentimentAnalyzer("wonrax/phobert-base-vietnamese-sentiment")
    ...
    try:
        ...
        for food_id, score in raw_recs:
            ...
            if not food_info.empty:
                ...
                category_id = (
                    int(food["categoryid"])
                    if "categoryid" in food and not pd.isna(food["categoryid"])
                    else 0
                )
                category_name = (
                    str(food["category_name"]) if "category_name" in food else "Unknown"
                )
                ...
                category_info = CategoryInfo(
                    id=category_id, categoryName=category_name, img=category_img
                )
                ...
                rec = FoodRecommendation(
                    ...
                    score=float(score),
                    description=(
                        str(food["description"]) if "description" in food else None
                    ),
                )
...
def get_user_stats(user_id: int):
    ...
    try:
        ...
        if len(user_orders) > 0:
            ...
            fav_category = model.foods_df[model.foods_df["food_id"].isin(user_foods)][
                "category_name"
            ].mode()
        ...
        return {
            ...
            "total_items": (
                int(user_orders["quantity"].sum()) if len(user_orders) > 0 else 0
            ),
            "total_spent": (
                float((user_orders["quantity"] * user_orders["unit_price"]).sum())
                if len(user_orders) > 0
                else 0
            ),
            "total_reviews": len(user_reviews),
            "avg_rating": (
                float(user_reviews["rating"].mean()) if len(user_reviews) > 0 else 0
            ),
...
def get_popular_foods(limit: int = 10):
    ...
    try:
        ...
        popular_foods = sorted(
            model.popularity_scores.items(), key=lambda x: x[1], reverse=True
        )[:limit]
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "category_name": (
                    str(food["category_name"]) if "category_name" in food else None
                ),
                "category_img": (
                    str(food["category_img"])
                    if "category_img" in food and not pd.isna(food["category_img"])
                    else "NOT FOUND"
                ),
...
async def search_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        similarities, indices = vectorizer.index.search(query_vector, k)
...
# 4.x Phân tích cảm xúc đánh giá
@app.post("/sentiment")
